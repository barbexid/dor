...
def show_package_details(api_key, tokens, package_option_code, is_enterprise, option_order=-1):
    ...
    if not package:
        print_panel("Error", "Gagal memuat detail paket.")
        pause()
        return "BACK"
    ...
    while True:
        ...
        if choice == "99":
            ...
            print_panel("Error", "Pilihan tidak valid. Pastikan input sesuai dengan nomor yang ada di menu.")
            #pause()
...
def get_packages_by_family(
    ...
    if not tokens:
        print_panel("Error", "Tidak ditemukan token pengguna aktif.")
        pause()
        return "BACK"
    ...
    if not data:
        print_panel("Error", "Gagal memuat data paket family.")
        pause()
        return "BACK"
    ...
    while True:
        ...
        if not pkg_choice.isdigit():
            print_panel("Error", "Input tidak valid. Masukkan nomor paket.")
            pause()
            continue

        selected_pkg = next((p for p in packages if p["number"] == int(pkg_choice)), None)
        if not selected_pkg:
            print_panel("Error", "Paket tidak ditemukan. Silakan masukkan nomor yang benar.")
            pause()
            continue
...
def fetch_my_packages():
    ...
    while True:  # Loop utama untuk reload menu
        ...
        if res.get("status") != "SUCCESS":
            print_panel("Error", "Gagal mengambil paket.")
            pause()
            return None
