...
def login_prompt(api_key: str):
    ...
    while True:
        ...
        if raw_input == "00":
            print_panel("Dibatalkan", "Login dibatalkan oleh pengguna.")
            return None, None

        phone_number = normalize_number(raw_input)
        if len(phone_number) < 10 or len(phone_number) > 14:
            print_panel("Error", "Nomor tidak valid. Coba lagi atau ketik 00 untuk batal.")
            continue
        break
    ...
    try:
        ...
        while attempts < max_attempts:
            ...
            if not otp.isdigit() or len(otp) != 6:
                print_panel("Error", "OTP tidak valid. Harus 6 digit angka.")
                attempts += 1
                continue
            ...
            if tokens:
                ...
                print_panel("Error", "OTP salah atau sudah kedaluwarsa.")
                attempts += 1
        ...
        print_panel("Error", "Terjadi kesalahan saat login.")
        pause()
        return None, None
    ...
    while in_account_menu:
        ...
        if is_adding_user and len(users) >= MAX_FREE_ACCOUNTS and not is_unlocked:
            ...
            if unlock_input != UNLOCK_CODE:
                print_panel("Gagal", "Kode unlock salah. Tidak dapat menambah akun.")
                pause()
                is_adding_user = False
                continue
            else:
        ...
        if AuthInstance.get_active_user() is None or is_adding_user:
            ...
            if not refresh_token:
                print_panel("Dibatalkan", "Login dibatalkan. Kembali ke menu akun.")
                pause()
                is_adding_user = False
                continue
        ...
        if user_input == "00":
            ...
            if nomor_input.isdigit():
                ...
                if 1 <= nomor <= len(users):
                    ...
                    if new_name:
                        ...
                        print_panel("Error", "Nama tidak boleh kosong.")
                        pause()
                else:
                    print_panel("Error", f"Nomor akun di luar jangkauan (1 - {len(users)}).")
                    pause()
            else:
                print_panel("Error", "Input tidak valid. Masukkan angka atau 00 untuk batal.")
                pause()


        elif user_input.upper() == "H":
            if not users:
                print_panel("Error", "Tidak ada akun untuk dihapus.")
                pause()
                continue

            nomor_input = console.input(f"[{theme['text_sub']}]Nomor akun yang ingin dihapus (1 - {len(users)}):[/{theme['text_sub']}] ").strip()
            if nomor_input == "00":
                print_panel("Dibatalkan", "Penghapusan akun dibatalkan.")
                pause()
                continue
            ...
            print_panel("Error", "Input tidak dikenali. Silakan pilih opsi yang tersedia.")
            pause()
